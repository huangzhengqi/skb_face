 ...
 * @ Description: 三级代理商控制层
 **/
@Slf4j
@RestController
@RequestMapping("/auth/company/third_agent")
@Api(value = "ThirdAgentController",tags = "三级代理商控制层")
public class ThirdAgentController extends BaseController<Company> {
    ...
    public Resp<String> isable(@ApiParam(value = "公司id") @RequestParam(value = "id") String id,
        ...
        Company company = companyService.findByIdAndType(id,Company.Type.THIRDAGENT.getCode());
        if (company == null) {
    ...
    public Resp listRewrite(Company model, PageVo pageVo, @UserId String userId) {
        return Resp.success(companyCommonService.listRewrite(model, pageVo, userId,Company.Type.THIRDAGENT.getCode()));
    }

    @GetMapping("/list_rewrite_count")
    public Resp listRewriteCount(Company model, @UserId String userId) {
        return Resp.success(companyCommonService.listRewriteCount(model, userId, Company.Type.THIRDAGENT.getCode()));
    }

    @PostMapping("/save_rewrite")
    public Resp saveRewrite(@Valid Company model, @UserId String userId) {
        return Resp.success(companyService.saveAgent(model, userId, Company.Type.THIRDAGENT.getCode()));
    }
    ...
    public Resp audit(String id, Integer status) {
        return Resp.success(companyService.audit(id, status, Role.Type.THIRDAGENT.getCode()));
    }
